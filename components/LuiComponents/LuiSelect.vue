<template>
  <div :class="computedClasses.selectWrapper">
    ...
    >
      ...
      <span v-else-if="!customOptions.length"> {{ selectedOption }} </span>
      <div v-else>
        <img
          width="20"
          height="20"
          :src="selectedCustomOption.icon"
          :alt="selectedCustomOption.text"
        />
      </div>
      <component
        :is="selectAppend.tag"
    ...
    >
      ...
      <div v-if="customOptions.length > 0" class="p-2">
        <div
          v-for="(item, index) in customOptions"
          :key="index"
          class="px-4 cursor-pointer hover:bg-primary-100 rounded-sm flex items-center"
          @click="selectCustomOption(item)"
        >
          <img width="20" height="20" :src="item.icon" :alt="item.text" />
          <span class="ml-1 p-2 text-secondary-600"> {{ item.text }} </span>
        </div>
      </div>
    </ul>
  </div>
...
export default {
  ...
  emits: ['update:modelValue', 'onCustomChange'],
  setup(props, { emit }) {
    ...
    const selectedCustomOption = ref({})
    ...
    if (props.customOptions.length) {
      selectedCustomOption.value = props.customOptions[0]
    }
    ...
    function selectCustomOption(option) {
      selectedCustomOption.value = option
      emit('onCustomChange', option)
    }
